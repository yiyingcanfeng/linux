...
MONGO_VERSIONS=("4.0" "3.6" "3.4" "3.2" "3.0")
STABLE_VERSION="4.0"
...
baseurl=https://mirrors.tuna.tsinghua.edu.cn/mongodb/yum/el6-${mgver}/
repo_gpgcheck=0
gpgcheck=0
enabled=1
sslverify=0
...
baseurl=https://mirrors.tuna.tsinghua.edu.cn/mongodb/yum/el7-${mgver}/
repo_gpgcheck=0
gpgcheck=0
enabled=1
sslverify=0
...
if [[ -z ${DRY_RUN:-} ]]; then
	param=""
	for repo_mongodb in $(crudini --get $cfg | grep el); do
		param=${param}" -r "${repo_mongodb}
	done
